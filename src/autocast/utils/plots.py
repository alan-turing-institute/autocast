...
def plot_spatiotemporal_video(  # noqa: PLR0915, PLR0912
    ...
    Args:
        ...
        pred (array_like): Optional predicted tensor of shape (B, T, W, H, C).
        batch_idx (int): Which batch index to visualize (default: 0).
        fps (int, optional): Frames per second for the video (default: 5).
        vmin (float, optional): Minimum value for color scale (default: auto from data).
        vmax (float, optional): Maximum value for color scale (default: auto from data).
        cmap (str, optional): Colormap to use (default: "viridis").
        save_path (str, optional): Optional path to save the video (e.g., "output.mp4").
        title (str, optional): Title for the video (default: "Ground Truth vs Prediction").
        colorbar_mode ({"none", "row", "column", "all"}): Select how colorbars (and underlying color scales) are shared for the
        ...
        preserve_aspect (bool): If True, resize each subplot panel to match the spatial WxH ratio of the
    ...
    Returns:
        animation.FuncAnimation: Animation object that can be displayed in notebooks.
    """
...
def compute_metrics_from_dataloader(
    ...
    Args:
        ...
        metric_fns (dict[str, Callable[[], Metric]]): Dictionary of functions that return fresh metric instances, keyed by metric
            name.
        predict_fn (Callable): Custom function (batch) -> (preds, trues) for cases like rollout or simply
        ...
        windows (list[tuple[int, int] | None], optional): List of (t_start, t_end) windows to evaluate. None means use all timesteps.
        ...
        return_per_batch (bool): If True, also return a list of dictionaries containing metrics for each batch.
    Returns:
...
def compute_metrics_per_timestep_from_dataloader(  # noqa: PLR0912, PLR0915
    ...
    Args:
        ...
        metric_fns (dict[str, Callable[[], Metric]]): Metric factory functions. Metrics should return (1, C) when updated with
            (B, 1, S, C) and reduce_all=False (deterministic metrics) or be
            MultiCoverage (expanded to one key per alpha).
        predict_fn (Callable): (batch) -> (preds, trues) returning tensors of shape (B, T, S, C) or
            (B, T, S, C, M). Returns None, None to skip a batch.
        max_timesteps (int | None, optional): Cap the number of timesteps (uses min over batches otherwise).
    Returns:
        dict[str, np.ndarray]: Keys are metric names (and coverage_0.05, coverage_0.10, ... for
...
def compute_coverage_scores_from_dataloader(
    ...
    Args:
        dataloader (DataLoader): DataLoader that yields batches.
        model (nn.Module, optional): Model with forward(batch) that returns predictions with ensemble dimension.
            Either model or predict_fn must be provided.
        predict_fn (Callable, optional): Custom function (batch) -> (preds, trues) for cases like rollout.
            Either model or predict_fn must be provided.
        coverage_levels (list[float], optional): Coverage levels to evaluate (default: 0.05 to 0.95).
        windows (list[tuple[int, int] | None], optional): List of (t_start, t_end) windows to evaluate. None means use all timesteps.
            If multiple windows provided, evaluates each independently.
        return_tensors (bool): If True, also return concatenated (pred, true) tensors.
    Returns:
...
def plot_coverage(
    ...
    Args:
        pred (TensorBTSCM): Ensemble predictions (last dimension is ensemble members).
        true (TensorBTSC): Ground truth tensor.
        coverage_levels (list[float], optional): Coverage levels to evaluate (default: 0.05 to 0.95).
