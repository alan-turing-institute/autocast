...
if (( LOLA_EPOCH_SIZE % EFFECTIVE_GLOBAL_BATCH_SIZE != 0 )); then
    echo "LOLA_EPOCH_SIZE must divide effective global batch size." >&2
    echo "  LOLA_EPOCH_SIZE: ${LOLA_EPOCH_SIZE}" >&2
    echo "  effective global batch: ${EFFECTIVE_GLOBAL_BATCH_SIZE}" >&2
    exit 1
fi

LOLA_STEPS_PER_EPOCH="$((LOLA_EPOCH_SIZE / EFFECTIVE_GLOBAL_BATCH_SIZE))"
