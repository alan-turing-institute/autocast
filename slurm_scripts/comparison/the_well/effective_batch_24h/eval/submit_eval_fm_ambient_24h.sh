...
set -euo pipefail
# Evaluate the RB effective-batch 24h FM-in-ambient EPD run
# (the_well_rayleigh_benard_effbatch24h_fm_ambient_b256).
#
# NOTE: this run is not trained yet. RUN_PATTERNS globs over outputs/*/ so this
# script auto-resolves the output dir once the run lands; until then it prints a
# "resolved_config.yaml missing" skip. Pin the date dir explicitly once trained
# (e.g. outputs/2026-05-22/...) to match the other 24h eval scripts.
#
...
#
# Batch size: RB ambient resolution is 512x128 (16x the 64x64 Well-2D basis) and
# FM integrates flow_ode_steps per rollout step, so this is the most expensive
# of the four combinations. EVAL_BATCH_SIZE=1; drop nothing further is possible,
# so reduce eval.max_rollout_steps or n_members if it OOMs. No AE decode and no
# chunking are needed here (identity encoder/decoder).
#
...
RUN_PATTERNS=(
    "outputs/*/the_well_rayleigh_benard_effbatch24h_fm_ambient_b256"
)
...
for run_dir in "${RUN_DIRS[@]}"; do
    if [[ ! -f "${run_dir}/resolved_config.yaml" ]]; then
        echo "Skipping ${run_dir}: resolved_config.yaml missing (run not trained yet?)" >&2
        continue
    fi
    ...
    for run_dry in "${RUN_DRY_STATES[@]}"; do
        ...
        echo "Submitting RB eff-batch 24h FM-ambient eval (mode=ambient, EPD)"
        echo "  mode: ${run_label}"
