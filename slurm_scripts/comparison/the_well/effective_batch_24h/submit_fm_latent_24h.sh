...
# FM in cached LoLA latent space.
#
...
EXPERIMENT_NAME="the_well_rayleigh_benard_effbatch24h_fm_latent_b256"
RUN_ID="rb_eff24_fm_latent_b256"
...
resolve_cosine_epochs() {
    ...
    if [[ -n "${COSINE_EPOCHS_FM_LATENT:-}" ]]; then
        printf '%s\n' "${COSINE_EPOCHS_FM_LATENT}"
        return 0
    fi
    ...
    if [[ -z "${timing_ckpt}" ]]; then
        ...
        echo "Run ${SCRIPT_DIR}/submit_fm_latent_timing.sh first, or set COSINE_EPOCHS_FM_LATENT=<epochs>." >&2
        return 1
    fi
