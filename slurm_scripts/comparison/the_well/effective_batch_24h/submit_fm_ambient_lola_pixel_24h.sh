...
# FM in ambient space with the LOLA pixel-space ViT hyperparameters.
#
# Effective global batch = 256/GPU * 4 GPUs = 1024. Epochs are fixed-size
# budget epochs of 64 train batches by default, not full dataset passes.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../../../.." && pwd)"
DATASETS_ROOT="${AUTOCAST_DATASETS:-${REPO_ROOT}/datasets}"

EXPERIMENT="the_well/rayleigh_benard/fm_vit_lola_pixel_ambient_identity"
EXPERIMENT_NAME="the_well_rayleigh_benard_effbatch24h_fm_ambient_lola_pixel_b256"
RUN_ID="rb_eff24_fm_ambient_lola_pixel_b256"
...
resolve_cosine_epochs() {
    ...
    if [[ -n "${COSINE_EPOCHS_FM_AMBIENT_LOLA_PIXEL:-}" ]]; then
        printf '%s\n' "${COSINE_EPOCHS_FM_AMBIENT_LOLA_PIXEL}"
        return 0
    fi
    ...
    if [[ -z "${timing_ckpt}" ]]; then
        ...
        echo "Run ${SCRIPT_DIR}/submit_fm_ambient_lola_pixel_timing.sh first, or set COSINE_EPOCHS_FM_AMBIENT_LOLA_PIXEL=<epochs>." >&2
        return 1
    fi
...
for run_dry in "${RUN_DRY_STATES[@]}"; do
    ...
    echo "  mode: ${run_label}"
    echo "  method: FM ambient LOLA pixel ViT"
