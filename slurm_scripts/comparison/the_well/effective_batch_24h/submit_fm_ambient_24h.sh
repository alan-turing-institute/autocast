...
# FM in ambient space via identity encoder/decoder.
#
...
EXPERIMENT="the_well/rayleigh_benard/fm_vit_large_ambient_identity"
EXPERIMENT_NAME="the_well_rayleigh_benard_effbatch24h_fm_ambient_b256"
RUN_ID="rb_eff24_fm_ambient_b256"
...
resolve_cosine_epochs() {
    ...
    if [[ -n "${COSINE_EPOCHS_FM_AMBIENT:-}" ]]; then
        printf '%s\n' "${COSINE_EPOCHS_FM_AMBIENT}"
        return 0
    fi
    ...
    if [[ -z "${timing_ckpt}" ]]; then
        ...
        echo "Run ${SCRIPT_DIR}/submit_fm_ambient_timing.sh first, or set COSINE_EPOCHS_FM_AMBIENT=<epochs>." >&2
        return 1
    fi
