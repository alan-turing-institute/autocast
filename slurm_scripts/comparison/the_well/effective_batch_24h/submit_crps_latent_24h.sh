...
EXPERIMENT_NAME="the_well_rayleigh_benard_effbatch24h_crps_latent_b32_m8"
...
resolve_cosine_epochs() {
    ...
    if [[ -n "${COSINE_EPOCHS_CRPS_LATENT:-}" ]]; then
        printf '%s\n' "${COSINE_EPOCHS_CRPS_LATENT}"
        return 0
    fi
    ...
    if [[ -z "${timing_ckpt}" ]]; then
        ...
        echo "Run ${SCRIPT_DIR}/submit_crps_latent_timing.sh first, or set COSINE_EPOCHS_CRPS_LATENT=<epochs>." >&2
        return 1
    fi
