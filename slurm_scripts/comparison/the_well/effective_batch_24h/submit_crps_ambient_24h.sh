...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-the_well_rayleigh_benard_effbatch24h_crps_ambient_b32_m8}"
RUN_ID="${RUN_ID:-rb_eff24_crps_ambient_b32_m8}"
...
resolve_cosine_epochs() {
    ...
    if [[ -n "${COSINE_EPOCHS_CRPS_AMBIENT:-}" ]]; then
        printf '%s\n' "${COSINE_EPOCHS_CRPS_AMBIENT}"
        return 0
    fi
    ...
    if [[ -z "${timing_ckpt}" ]]; then
        ...
        echo "Run ${SCRIPT_DIR}/submit_crps_ambient_timing.sh first, or set COSINE_EPOCHS_CRPS_AMBIENT=<epochs>." >&2
        return 1
    fi
