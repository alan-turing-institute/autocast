...
find_timing_checkpoint() {
    ...
    find outputs -path "*/timing_planned_updates_02/${run_id}/timing.ckpt" \
        | sort | tail -n 1
}
...
for datamodule in "${!EXPERIMENTS[@]}"; do
    ...
    if ! cosine_epochs="$(resolve_cosine_epochs "${datamodule}")"; then
        echo "No timing-derived cosine_epochs available for ${datamodule}" >&2
        exit 1
    fi
    if [[ -z "${cosine_epochs}" ]]; then
        echo "Could not parse trainer.max_epochs for ${datamodule}" >&2
        exit 1
    fi
    ...
    for run_dry in "${RUN_DRY_STATES[@]}"; do
        ...
        echo "Submitting planned updates batch 02 training"
        echo "  mode: ${run_label}"
        echo "  run_id: ${run_id}"
