"""Preview or submit the measured diffusion extensions from their manifest."""
...
RUN_IDS = {"latent_diffusion_ad", "latent_diffusion_gs", "latent_diffusion_gpe"}
...
def _validate_config(run: dict) -> None:
    ...
    cfg = compose(config_name="processor", overrides=run["overrides"])
    ...
    # The original timing config omitted this value. ProgressModelCheckpoint
    # defaults it to zero for fractional step snapshots; production is explicit.
    for callback in expected_trainer["callbacks"]:
        if (
            callback.get("_target_")
            == "autocast.callbacks.checkpoint.ProgressModelCheckpoint"
            and callback.get("every_n_train_steps_fraction") is not None
        ):
            callback.setdefault("every_n_epochs", 0)
    expected_data = OmegaConf.to_container(timed.datamodule, resolve=True)
    actual_data = resolved["datamodule"]
    if not isinstance(expected_data, dict) or not isinstance(actual_data, dict):
        msg = "Expected datamodule mappings in the timing and production configs"
        raise TypeError(msg)
    # The old checkout's outputs symlink and the shared filesystem path must
    # identify the same existing latent cache, not merely similarly named data.
    expected_data["data_path"] = str(
        Path(expected_data["data_path"]).resolve(strict=True)
    )
    actual_data["data_path"] = str(Path(actual_data["data_path"]).resolve(strict=True))
    expected = {
        "model": OmegaConf.to_container(timed.model, resolve=True),
        "datamodule": expected_data,
...
def _prepare_launches(run_group: str) -> list[tuple[Path, list[str]]]:
    ...
    runs = [run for run in manifest["runs"] if run["kind"] == "processor"]
    if len(runs) != len(RUN_IDS) or {run["run_id"] for run in runs} != RUN_IDS:
        msg = "Expected exactly the AD, GS and GPE diffusion production runs"
