...
set -euo pipefail
# Eval submitter for the cross-dataset plain-CRPS ablation runs.
#
# This mirrors the CRPS-ambient profile from submit_eval_planned_04.sh, but
# targets the plain CRPS m=8 ViT ablation for GS, GPE, and AD. Fill the RUNS
# table after planned_05 production runs have concrete output directories.
#
...
RUNS=(
    # "plain_crps_m8_gs|gray_scott|outputs/REPLACE_ME/planned_05/crps_gs64_vit_azula_large_REPLACE_ME"
    # "plain_crps_m8_gpe|gpe_laser_only_wake|outputs/REPLACE_ME/planned_05/crps_gpe64_vit_azula_large_REPLACE_ME"
    # "plain_crps_m8_ad|advection_diffusion|outputs/REPLACE_ME/planned_05/crps_ad64_vit_azula_large_REPLACE_ME"
)
...
submit_crps_ambient() {
    ...
    for run_dry in "${RUN_DRY_STATES[@]}"; do
        ...
        echo "Submitting plain-CRPS ambient eval (best multi-Winkler)"
        echo "  mode: ${run_label}"
        echo "  run_id: ${run_id}"
...
if (( ${#RUNS[@]} == 0 )); then
    echo "No planned batch 05 run dirs configured yet; fill RUNS after production completes."
    exit 0
fi
...
for run_spec in "${RUNS[@]}"; do
    ...
    if ! grep -q "autocast.losses.ensemble.CRPSLoss" "${run_dir_abs}/resolved_config.yaml"; then
        echo "Skipping ${run_id}: run is not configured with CRPSLoss" >&2
        continue
    fi
