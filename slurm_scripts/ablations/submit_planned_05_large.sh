...
# 24h production jobs for planned ablation batch 05.
#
# Run submit_planned_05_timing.sh first. This script resolves each
...
TIMEOUT_MIN=1439
RUN_GROUP="$(date +%Y-%m-%d)/planned_05"
...
find_timing_checkpoint() {
    ...
    find outputs -path "*/timing_planned_05/${run_id}/timing.ckpt" | sort | tail -n 1
}
...
for run_spec in "${RUNS[@]}"; do
    ...
    for run_dry in "${RUN_DRY_STATES[@]}"; do
        ...
        echo "Submitting planned batch 05 production run"
        echo "  mode: ${run_label}"
        echo "  run_id: ${run_id}"
        echo "  kind: ${kind}"
