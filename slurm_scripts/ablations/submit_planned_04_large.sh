...
# 24h production jobs for planned ablation batch 04.
#
# Run submit_planned_04_timing.sh first. This script resolves each
...
TIMEOUT_MIN=1439
RUN_GROUP="$(date +%Y-%m-%d)/planned_04"
RUN_DRY_STATES=("true" "false")

# run_id|kind|datamodule|local_experiment
RUNS=(
    "fair_crps_m8_gs|epd|gray_scott|ablations/crps_variants/gray_scott/crps_vit_fair"
    "fair_crps_m8_gpe|epd|gpe_laser_only_wake|ablations/crps_variants/gpe_laser_wake_only/crps_vit_fair"
    "fair_crps_m8_ad|epd|advection_diffusion|ablations/crps_variants/advection_diffusion/crps_vit_fair"
)
...
find_timing_checkpoint() {
    ...
    find outputs -path "*/timing_planned_04/${run_id}/timing.ckpt" | sort | tail -n 1
}
...
for run_spec in "${RUNS[@]}"; do
    ...
    for run_dry in "${RUN_DRY_STATES[@]}"; do
        ...
        echo "Submitting planned batch 04 production run"
        echo "  mode: ${run_label}"
        echo "  run_id: ${run_id}"
        echo "  kind: ${kind}"
