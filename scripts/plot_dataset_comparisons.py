...
ARCHITECTURE_PREFIXES = (
    "flow_matching_vit", "flow_matching_large", "flow_matching", "fno_concat", "fno",
    "vit_latent", "vit_large", "vit", "diffusion_vit", "diffusion", "unet_large", "unet",
    "swin", "unet_azula_large", "unet_azula_small", "unet_large_concat", "unet_small_concat",
    "swin_large", "swin_small", "vit_azula_large", "vit_azula_small",
)

MODEL_FAMILY_DISPLAY_LABELS = {
    "flow_matching_vit": "FM ViT", "fno_concat": "FNO", "flow_matching": "FM U-Net",
    "flow_matching_large": "FM U-Net (large)", "vit_large": "ViT (large)", "vit": "ViT (small)",
    "vit_latent": "ViT latent", "diffusion": "Diffusion U-Net", "diffusion_vit": "Diffusion ViT",
    "unet_azula_large": "Azula U-Net (large)", "unet_azula_small": "Azula U-Net (small)",
    "unet_large_concat": "U-Net (large)", "unet_small_concat": "U-Net (small)",
    "swin_large": "Swin (large)", "swin_small": "Swin (small)",
    "vit_azula_large": "Azula ViT (large)", "vit_azula_small": "Azula ViT (small)",
}
...
def resolve_results_root(outputs_dir: str) -> Path:
...
def parse_loss_dataset_arch(run_name: str | None) -> tuple[str | None, str | None, str | None]:
    if not run_name: return None, None, None
...
def normalize_dataset_module(dataset: str | None) -> str | None:
    if pd.isna(dataset) or not dataset: return None
...
def load_single_run_metrics(run_dir: Path) -> dict:
    ...
    if p_eval.exists():
        ...
        if not em.empty and "window" in em.columns:
            ...
            if "batch_idx" in em.columns:
                ...
                if ov.empty: ov = em[em["window"] == "all"]
            else:
                ov = em[em["window"] == "all"]
            if not ov.empty:
                rr = ov.iloc[0]
                for col in em.columns:
                    if col in {"window", "batch_idx"}: continue
    ...
    if p_meta.exists():
        ...
        if not md.empty and {"category", "metric", "value"}.issubset(md.columns):
            ...
            for metric, out_col in [("model_total", "params_model_total"), ("processor_total", "params_processor_total")]:
                s = params.loc[params["metric"] == metric, "value"]
                if not s.empty: row[out_col] = pd.to_numeric(s.iloc[0], errors="coerce")
    return row

def assign_model_scale(df_in: pd.DataFrame) -> pd.Series:
    out = pd.Series("large", index=df_in.index, dtype="object")
    if MODEL_SCALE_PARAM_COL not in df_in.columns: return out
    ...
    if len(group_cols) < 2: return out
    ...
    for _, g in df_in.groupby(group_cols, dropna=False):
        ...
        for ii in p[(p != low) & (p != high)].index:
            val = p.loc[ii]
            out.loc[ii] = "small" if pd.notna(val) and abs(val - low) <= abs(val - high) else "large"
    return out
...
def get_hue_and_lightness(pg: str, df_in: pd.DataFrame, explicit_groups: list[list[str]] | None):
    """
...
def plot_group_display_label(pg: str) -> str:
    parts = pg.split("__")
    arch, loss, scale = parts[0], parts[1] if len(parts) > 1 else "?", parts[2] if len(parts) > 2 else "?"
    model_l = MODEL_FAMILY_DISPLAY_LABELS.get(arch, arch)
    loss_l = {"crps": "CRPS", "diff": "FM"}.get(loss, loss)
    return f"{model_l} · {scale} · {loss_l}"

def extract_valid_plot_groups_from_run_names(run_names: list[str], df_in: pd.DataFrame) -> set[str]:
...
def build_family_style(df_in: pd.DataFrame, explicit_groups: list[list[str]] | None = None) -> dict:
    styles = {}
    ...
    if explicit_groups and len(explicit_groups) > 0:
        ...
        for i, group_runs in enumerate(explicit_groups):
            ...
            if not group_pgs: continue

            # Vary lightness across the group's plot groups
            n = len(group_pgs)
            for j, pg in enumerate(group_pgs):
                if pg in styles: continue # Conflict resolution (first claims)
                # j=0 -> normal, j=1 -> lighter, j=2 -> darker etc
                if n == 1:
                    c = base_color
                elif n == 2:
                    c = _mix_with_white(base_color, 0.4) if j == 0 else _mix_with_black(base_color, 0.2)
                else:
                    frac = (j / (n-1)) - 0.5 # -0.5 to 0.5
                    c = _mix_with_white(base_color, frac*1.2) if frac > 0 else _mix_with_black(base_color, -frac*0.8)
...
def grouped_bar(df_in: pd.DataFrame, metric: str, title: str, ylabel: str, out_dir: Path, styles: dict):
    if metric not in df_in.columns: return
    ...
    for i, fam in enumerate(families):
        ...
        ax.bar(x + offs, vals, width=width, label=style["label"], color=style["color"],
               edgecolor="0.25", linewidth=0.6, linestyle=style.get("linestyle", "-"))
...
def plot_coverage_calibration_panel(df_in: pd.DataFrame, results_root: Path, out_dir: Path, styles: dict):
    curves = []
    base = df_in.dropna(subset=["run_path", "dataset_label", "plot_group"]).drop_duplicates()
    for _, r in base.iterrows():
        for w in WINDOW_ROWS:
            fn = "test_coverage_window_all.csv" if w=="all" else f"rollout_coverage_window_{w}.csv"
            p = results_root / r["run_path"] / "eval" / fn
            if p.exists():
                c = pd.read_csv(p)
                if not c.empty and {"coverage_level", "observed_mean"}.issubset(c.columns):
                    c["window"] = w
    ...
    fig, axes = plt.subplots(nrows, ncols, figsize=(4.0*ncols, 2.3*nrows), sharex=True, sharey=True, squeeze=False)
    ...
    for i, w in enumerate(WINDOW_ROWS):
        for j, ds_label in enumerate(datasets):
            ...
            for fam in groups:
                ...
                # Plot family mean
                mean_curve = sf.groupby("coverage_level", as_index=False)["observed_mean"].mean().sort_values("coverage_level")
                ax.plot(mean_curve["coverage_level"], mean_curve["observed_mean"], color=st["color"], lw=2, linestyle=st.get("linestyle", "-"))
            if i == 0: ax.set_title(ds_label)
            if j == 0: ax.set_ylabel(w)
            ax.grid(alpha=0.2)
    
    legend_handles = [plt.Line2D([0], [0], color=styles[g]["color"], ls=styles[g].get("linestyle", "-"), lw=2, label=styles[g]["label"]) for g in groups if g in styles]
    fig.legend(legend_handles, [h.get_label() for h in legend_handles], loc="upper center", bbox_to_anchor=(0.5, 0.995), ncol=4, fontsize=8, frameon=False)
    plt.tight_layout(rect=(0, 0, 1, 0.975))
    save_fig(fig, out_dir, "coverage_calibration_panel.png")

def plot_lead_time_panel(df_in: pd.DataFrame, metrics: list[str], results_root: Path, out_dir: Path, name: str, styles: dict):
    rows = []
    base = df_in.dropna(subset=["run_path", "dataset_label", "plot_group"]).drop_duplicates()
    for r in base.itertuples(index=False):
        p = results_root / str(r.run_path) / "eval" / "rollout_metrics_per_timestep_channel_0.csv"
        if not p.exists(): continue
        raw = pd.read_csv(p, index_col=0)
        if raw.empty: continue
        long = raw.reset_index().rename(columns={raw.index.name or "index": "metric"})
        long = long.melt(id_vars="metric", var_name="timestep", value_name="value").dropna()
    ...
    fig, axes = plt.subplots(nrows, ncols, figsize=(3.6*ncols, 2.7*nrows), sharex=True, sharey=False, squeeze=False)
    ...
    for r, metric in enumerate(metrics):
        ...
        for c, ds_label in enumerate(datasets):
            ...
            for fam in groups:
                ...
                if sf.empty: continue
                agg = sf.groupby("timestep", as_index=False)["value"].agg(["mean", "std", "count"]).sort_values("timestep")
                st = styles.get(fam, {"color":"k"})
                
                m = agg["mean"].clip(lower=0, upper=1) if is_cov else agg["mean"].clip(lower=1e-6)
                if not is_cov: vals.extend(m.dropna().tolist())
                ax.plot(agg["timestep"], m, color=st["color"], lw=2, linestyle=st.get("linestyle", "-"))
                if (agg["count"] > 1).any():
                    y1 = (agg["mean"] - agg["std"].fillna(0)).clip(lower=0, upper=1) if is_cov else (agg["mean"] - agg["std"].fillna(0)).clip(lower=1e-6)
                    y2 = (agg["mean"] + agg["std"].fillna(0)).clip(lower=0, upper=1) if is_cov else (agg["mean"] + agg["std"].fillna(0)).clip(lower=1e-6)
                    if not is_cov: vals.extend(y1.dropna().tolist()); vals.extend(y2.dropna().tolist())
                    ax.fill_between(agg["timestep"], y1, y2, color=st["color"], alpha=0.15)
                    
            if r == 0: ax.set_title(ds_label)
            if r == nrows - 1: ax.set_xlabel("Lead time")
            if c == 0: ax.set_ylabel(metric)
            ax.grid(alpha=0.25)
            if is_cov: ax.set_ylim(0, 1)
            elif vals:
                ax.set_yscale("log")
                ymin = max(1e-6, min(vals) * 0.8) if vals else 1e-6
                ymax = max(vals) * 1.25 if vals else 10.0
                if not np.isfinite(ymin) or np.isnan(ymin): ymin = 1e-6
                if not np.isfinite(ymax) or np.isnan(ymax): ymax = 10.0
                ax.set_ylim(bottom=ymin, top=ymax)            
    handles = [plt.Line2D([0], [0], color=styles[g]["color"], ls=styles[g].get("linestyle", "-"), lw=2, label=styles[g]["label"]) for g in groups if g in styles]
    fig.legend(handles, [h.get_label() for h in handles], loc="upper center", bbox_to_anchor=(0.5, 0.98), ncol=4, fontsize=8, frameon=False)
...
def main():
    parser = argparse.ArgumentParser(description="Generate dataset comparison plots (extracted from notebook)")
    parser.add_argument("--results-dir", required=True, help="Path to collated results folder")
    parser.add_argument("--output-dir", required=True, help="Directory to save PNG plots")
    parser.add_argument("--run-group", action="append", nargs="+", help="Specify a group of runs. E.g. --run-group run1 run2 --run-group run3 run4")
    parser.add_argument("--runs", nargs="+", help="Explicit list of run directory names to include (if not using --run-group)")
    parser.add_argument("--datasets", nargs="+", help="Filter to specific dataset modules (e.g., ad64 gs64)")
    parser.add_argument("--models", nargs="+", help="Filter to specific model families (e.g., fno_concat unet_large_concat)")
    parser.add_argument("--metrics", nargs="+", default=["vrmse", "coverage"], help="Metrics to plot for rollouts and overall (default: vrmse coverage)")
    ...
    if explicit_groups:
        ...
        run_dirs = sorted([p for p in results_dir.iterdir() if p.is_dir() and (p/"eval"/"evaluation_metrics.csv").exists()])
    ...
    df["dataset_module"] = _parsed.map(lambda x: x[1]).fillna(df["dataset"].map(normalize_dataset_module))
    ...
    print(f"Found {len(df['dataset_label'].unique())} datasets and {len(df['plot_group'].unique())} model variants.")

    # Render overall bars
    for m in args.metrics:
        grouped_bar(df, f"overall_{m}", f"Overall {m.upper()}", m.upper(), out_dir, styles)

    # Render window bars
    for m in args.metrics:
        for w in ROLL_WINDOWS:
            grouped_bar(df, f"{m}_{w}", f"{m.upper()} in window {w}", m.upper(), out_dir, styles)
    ...
    if err_metric:
        plot_lead_time_panel(df, err_metric, results_dir, out_dir, "lead_time_panel_error.png", styles)
    if cov_metric:
        plot_lead_time_panel(df, cov_metric, results_dir, out_dir, "lead_time_panel_coverage.png", styles)
